...
document.getElementById('filter-form').addEventListener('input', filterTasks);
...
function filterTasks() {
    ...
    const table = document.getElementById('task-table'); // Asegúrate de que el ID de la tabla sea 'task-table'
    const rows = table.getElementsByTagName('tr');
    
    for (let i = 1; i < rows.length; i++) { // Empieza desde 1 para omitir el encabezado
        const cells = rows[i].getElementsByTagName('td');
        const taskPriority = cells[3].innerText.toLowerCase(); // Prioridad
        const taskStatus = cells[5].innerText.toLowerCase(); // Estado
        const taskText = cells[0].innerText.toLowerCase(); // Texto de la tarea (o lo que quieras buscar)
        ...
        if (textFilter && !taskText.includes(textFilter)) {
            showRow = false;
        }
